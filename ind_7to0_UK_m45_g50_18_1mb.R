...
plink.gen=read.plink("ind_7to0_UK_m45_g50_18.bed",
                     "ind_7to0_UK_m45_g50_18.bim",
                     "ind_7to0_UK_m45_g50_18.fam")
GEO.DIST.MAT <- read.table("ind_7to0_UK_m45_g50_Geo.txt", head = T) # Loading the Geographic distance matrix
data <- read.table("ind_7to0_UK_m45_g50_dates.txt", head = T) # Loading sample date information
...
for (a in 1:block.nr){
  ...
  plink.block.x=read.plink("ind_7to0_UK_m45_g50_18.bed",
                           "ind_7to0_UK_m45_g50_18.bim",
                           "ind_7to0_UK_m45_g50_18.fam",select.snps=b.df$block.x.names)
...
for (s in 1:block.nr){
  ...
  if (sum(is.na(block.array[,,s]))>0){
    s=30
    del.run=0
    while (sum(is.na(block.array[,,s]))>0){
      del.run=del.run+1
      print(paste("del run nr ", del.run))
      na.rows=rowSums(is.na(block.array[,,s]))
      na.rows.max=max(na.rows)
      ind.positions=which(rowSums(is.na(block.array[,,s]))<na.rows.max)
...
window.moves = 41 # Number of windows
...
for (m in seq(1,nr.run)){
  ...
  start.windows.at = 7000 # Starting time of the Sliding Window analyses (Years Ago)
...
saveRDS(results.array, file="ind_7to0_UK_m45_g50_18_1mb_res.RData")

if (length(diff.test)>0){
  saveRDS(diff.test, file="ind_7to0_UK_m45_g50_18_1mb_diff.RData")
}
